...
print('Class_Value', train_y.shape)
...
wd = './synthetic_JIGSAWS_datasets'
if not os.path.isdir(wd):
    os.mkdir(wd)

# runs the experiment 5 times
identifiers = ['JIGSAWS_cdgan_synthetic_dataset_r' + str(i) for i in range(2, 5)]
...
for identifier in identifiers:
    ...
    for i in range(len(train_x)):
        ...
        samples[i].reshape(-1, 1, 76)
    print("Samples shape:", len(samples[1][0]))
    '''
    ...
    train_seqs = samples[:36]
    vali_seqs = samples[36:38]
    test_seqs = samples[38:40]
    train_targets = labels[:36]
    vali_targets = labels[36:38]
    test_targets = labels[38:40]
    print("Train sequence shape:", train_seqs.shape)
    ...
    print(identifier)
    print('Printing train sequence:', train_seqs.shape)
    seq_length = train_seqs.shape[1]
    num_generated_features = 76  # train_seqs.shape[2]
    hidden_units_d = 100
    hidden_units_g = 100
    latent_dim = 10  # dimension of the random latent space
    cond_dim = train_targets.shape[1]  # dimension of the condition

    CG = tf.placeholder(tf.float32, [batch_size, train_targets.shape[0]])
    CD = tf.placeholder(tf.float32, [batch_size, train_targets.shape[0]])
